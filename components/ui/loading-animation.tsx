...
const RadialPulseLoader: React.FC<RadialPulseLoaderProps> = ({ 
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      for (let i = 0; i < numRays; i++) {
        ...
        const opacityByte = Math.max(0, Math.min(255, Math.floor(opacity * 255)));
        ctx.strokeStyle = `${color}${opacityByte.toString(16).padStart(2, '0')}`;
        ctx.lineWidth = 2;
        ctx.stroke();
  ...
  return (
    <div className="radial-pulse-loader flex flex-col items-center justify-center">
      <canvas ref={canvasRef}></canvas>
      {showText && (
        <div className="loader-text mt-2 font-mono text-[11px] text-slate-400 tracking-wider">
          {text}
        </div>
      )}
    </div>
